function factura (nroFactura,fecha,cliente,vehiculo,precio){//Se crea el objeto factura y se recibe sus valores nroFactura,etc
	this.nroFactura = nroFactura;//Se carga el nroFactura recibido en la variable local this.nroFactura
	this.fecha = fecha;//Se carga la fecha recibida en la variable local this.fecha Igual en los siguientes
...
function realizarVenta (factura){//Se crea la funcion venta,el cual recibe un objeto factura
	...
	if (venta.length < 1){//La primera vez que invoquemos la función se cumplirá esta condición
		...
		console.log("La factura se ha registrado correctamente");//Mostramos mensaje de confirmación
		...
		for (var i = 0; i < venta.length ; i++) {//Recorremos todo el array con la funcion array.length, que nos da el total de elementos del array
			if (venta[i].nroFactura == factura.nroFactura) {//En cada iteración, se compara el nroFactura que queremos ingresar con la factura almacenada en el array según el índice
				console.log("No se ingresó la factura");//Si se encuentra una factura con el mismo numero, se muestra el mensaje
				bandera = false;
		...
		if (bandera) {//Si la bandera es true
			...
			console.log("La factura se ha registrado correctamente");//Mensaje de confirmación
		}
	}
}
function listarVenta(){//Creamos la función
	venta.forEach(function(element){//Usamos el foreach, al igual que el "for" nos hará recorrer todo el array
...
function editarVenta(nroFactura,fecha,cliente,vehiculo,precio){//Creamos la función
	this.nroFactura = nroFactura;//Cargamos la informacion a modificar en variables locales
	this.fecha = fecha;
	this.cliente = cliente;
	this.vehiculo = vehiculo;
	this.precio = precio;
	...
	for (var i = 0; i < venta.length; i++) {//recorremos el array
		if(venta[i].nroFactura == nroFactura){//En el array, por cada iteracion, comparamos si hay algun nroFactura igual al que queremos modificar
			venta[i].fecha = fecha;//Si lo encontramos, cambiamos cada valor por los que estan en las variables locales
			venta[i].cliente = cliente;
			venta[i].vehiculo = vehiculo;
			venta[i].precio = precio;
			bandera = true;// Bandera ahora es true
			console.log("Se ha modificado la venta");//Mensaje de confirmación
			break;//Se rompe el ciclo, se acaba la iteración
		}
	}
	if (!bandera) {//Si la bandera es false
		console.log("No se ha encontrado la factura a modificar");//Mensaje de denegación
	}
}
function eliminarVenta(nroFactura){//Creamos la funcion y el funcionamiento es similar al de editar
	this.nroFactura = nroFactura;//Cargamos el nroFactura, ya que es el dato identificador
	var bandera = false;//Bandera false
	for (var i = 0; i < venta.length; i++) {//Recorremos el array
		if(venta[i].nroFactura == nroFactura){//Comparamos los nroFactura del array con el que queremos borrar
			venta.splice(i,1);//Si lo encontramos usando la función array.splice(indice,cantidad_a_elimiar) borramos la factura
			bandera = true;
	...
	if (!bandera) {//Si la bandera sigue siendo false
		console.log("No se ha encontrado la factura a eliminar");//Mensaje de denegación
	}
}
function mostrarDineroRecaudado(){//Creamos la función
	var dinero = 0;//variable para sumar todo el dinero
	for (var i = 0; i < venta.length; i++) {//Recorremos el array
		dinero = dinero + venta[i].precio;//Vamos sumando todos los campos "precio" de cada factura en el array a nuestra variable dinero
	}
	console.log("Se ha recaudado : " + dinero);//Mostramos el total
}
